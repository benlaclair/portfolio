...
import {
  ...
  socialGridFeatures,
  socialGridOutcomes,
} from "@/data/socialGridCaseStudyData";
...
export default function SocialGridCaseStudy() {
  const tool = TOOLS.find((t) => t.slug === "social-grid-generator")!;
  const currentIndex = TOOLS.findIndex((t) => t.slug === "social-grid-generator");
    ...
    <div className="relative pt-24 md:pt-32 pb-12 md:pb-24">
      ...
      <div className="px-6 md:px-12 mb-12 fade-up">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-ink tracking-tight mb-4">
            Social Grid Generator<span style={{ color: tool.accentColor }}>_</span>
          </h1>
      ...
      <div className="border-y border-white/8 mb-16 fade-up-1">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4">
          {socialGridStats.map((stat, i) => (
            <div
              key={stat.label}
              className={`px-6 md:px-10 py-8 md:py-10 ${i < socialGridStats.length - 1 ? "border-r border-white/8" : ""} ${i < 2 ? "border-b border-white/8 md:border-b-0" : ""}`}
            >
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <ScrollReveal className="mb-16 md:mb-20">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-ink tracking-tight leading-[1.1] mb-6 max-w-3xl">
              30+ posts a month, per client, in 5 minutes.
            </h2>
            <p className="text-sm text-muted leading-relaxed mb-8 max-w-2xl">
              Creating individual post graphics for 30+ posts per client per month across four platforms was a repetitive time sink. This tool parses a structured copy document, auto-assigns contextually relevant images using a weighted scoring system, renders a visual grid, and exports production-ready PNGs with smart file naming — all in one workflow.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/8 rounded-2xl overflow-hidden">
              {socialGridFeatures.map((f) => (
