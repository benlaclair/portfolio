...
import {
  ...
  prooferFeatures,
  prooferOutcomes,
} from "@/data/prooferCaseStudyData";
...
export default function ProoferCaseStudy() {
  const tool = TOOLS.find((t) => t.slug === "design-proofer")!;
  const currentIndex = TOOLS.findIndex((t) => t.slug === "design-proofer");
      ...
      <div className="px-6 md:px-12 mb-12 fade-up">
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-ink tracking-tight mb-4">
            {tool.name}<span style={{ color: tool.accentColor }}>_</span>
          </h1>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <ScrollReveal className="mb-16 md:mb-20">
            ...
            <h2 className="text-3xl md:text-4xl font-extrabold text-ink tracking-tight leading-[1.1] mb-6 max-w-3xl">
              Manual proofing doesn&rsquo;t scale. This does.
            </h2>
            <p className="text-sm text-muted leading-relaxed mb-8 max-w-2xl">
              Proofing designs against copy documents across 40+ client accounts was eating hours every week — and still missing errors. This tool takes a design PDF and a copy doc, sends both to Gemini with custom prompts, and returns structured findings by type, severity, and exact location. Triage what matters, then export a client-ready report.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/8 rounded-2xl overflow-hidden">
              {prooferFeatures.map((f) => (
